...
def delete_wines_screen():
    ...
    def remove_from_database():
        try:
            ...
            if '' in wine_info:
                ...
                answer = messagebox.askyesno(title='Confirm', message=f"You are sure you want to remove {wine_name}"
                                                                      f" from the database?")
                if answer == 'Yes':
                    with connection:
                        cursor.execute("INSERT INTO wine VALUES (:name, :grape, :region, :glass_price, :bottle_price)",
    ...
    addition_button = Button(text="Remove Wine", command=remove_from_database)
    addition_button.grid(column=1, row=6)
    addition_button.config(height=3, width=40)
